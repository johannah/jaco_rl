...
def evaluate(load_model_filepath):
    ...
    eval_env = maniulation.load('reach_site_features')
...
def train():
    ...
    for t in range(start_t, int(args.max_timesteps)):
        ...
        # Store data in replay buffer
        replay_buffer.add(state, action, reward, next_state, done, frame_compressed=frame_compressed, next_frame_compressed=next_frame_compressed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_timesteps", default=1e9, type=int, help='max time steps to train agent')
